...
- Top-level `T1.py` / `T3.py` / `sim_api.py` / `T4.py` / `T4_figures.py`
  are thin shims. Real code is in `cumcm/` (`common`, `t1`, `t3`, `t4`, `analysis`). Edit the
  package, not the shims; keep the same CLI behavior.
- Layering is a hard convention: deps only point down `common <- t1/t3/t4 <- analysis`.
...
- There is no test suite, linter config, or CI workflow. Treat `python -m cumcm.t3.covering` and
  byte-identical fixed-seed reruns as the checks.
