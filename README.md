...
## 问题二：第二个检测点怎么选（`T2.py`，不连模拟器，约 5 s）
...
结论（S₁ = 原点、θ₁ = 0°）：最优点 **S₂\* = (806, 598) m**，即距 S₁ **1004 m**、偏示向度
**+36.6°**（镜像解 −36.6° 等价）；最坏定位直径 **134.1 m**，相比只有一次测向的 1800 m
**缩小 13.4 倍**。候选区域（η = 10%，J ≤ 147.6 m）是以 S₁ 为心、半径 **948~1004 m**、
方位 θ₁ ± **[25°, 40°]** 的两条镜像弧带（合计 0.0208 km²）。策略一句话：
...
的定位四边形内嵌图（直径 134 m）；逐格数据在 `results/t2/t2_suitability.csv`，全部结论与
自检数字在 `results/t2/t2_second_site.json`。模型、算法、校验与灵敏度的完整说明见
`reports/问题二第二检测点候选区域.md`。
