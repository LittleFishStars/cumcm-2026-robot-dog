...
`jammers-py/` 里的 `data/`（演练统计库与行为日志，含参赛队号）没有随包提供：它是运行产物，
第一次演练时会自动重建。因此包内**离线命令与 `--practice` 演练模式都能跑**；只有「连官方模拟器」
的模式需要官方 `jammers-simulator.exe` 在本机运行，且只有 3 次机会，**评审时请勿使用**。
...
uv run -m t4 --practice 20 --seed 0 --robot-id <队号>   # 问题四：演练 20 局（同上；固定 seed 逐字节可复现）
...
- **参赛队号**：**一律运行时用 `--robot-id` 传入，代码里不含任何队号**（避免身份信息随源码/提交包扩散）。
  跑演练或官方模式必须显式给出，例如
  `uv run -m t3 --practice 1 --robot-id <12 位队号>`；不给会直接报错退出（退出码 2）。
  官方模式须与模拟器登录的队号一致；演练模式把它作为 `--team` 传给本地模拟器，取任意值都能跑。
  `t1`、`t2`、`--plan-only` 与上面四条自检都不连模拟器，不需要队号。
- **输出详略**：缺省只报每一步的关键结论（1~2 行）；加 `--verbose` 打印完整过程（覆盖校验明细、
  每局阶段统计、接口回显、逐局图路径等）；加 `--quiet` 只留产物路径，便于批处理。`t1` 本来就只
  输出两行结论（定位区域直径与最小覆盖圆），没有这两个开关。
