...
产物都在 `results/` 下，正式测评按次数编号：

```
results/t3/1/ 2/ 3/       正式测评的三次，各一局，meta.mode 是 official
results/t4/1/ 2/ 3/       同上。官方不返回真值，源数这类字段为 null
results/t2/               问题二的六个产物，python -m t2 离线生成，随时可复现
results/practice/t3/ t4/  本地演练那批，seed 2026 起 10 局，meta.mode 是 practice
```

六次正式测评的成绩（官方只回清除成功与否，不给源总数，所以这里只列清除了几个）：

| | 第 1 次 | 第 2 次 | 第 3 次 |
|---|---|---|---|
| 问题三 | 14 个 / 3722.4 s / 14562 m | 15 个 / 3126.4 s / 11737 m | 16 个 / 4058.8 s / 16419 m |
| 问题四 | 16 个 / 6406.0 s / 26505 m | 13 个 / 6809.1 s / 26155 m | 10 个 / 6503.3 s / 23331 m |
...
`api_calls.jsonl` 是唯一不可复现的产物，它写的是现实时间戳。正式测评更是一局一次机会，
那六次跑完什么样就存什么样。

**重跑一定要显式给 `--save-dir`**。缺省值是 `results/t3`、`results/t4`，那是放三次正式测评的
目录，直接往里头写会在编号目录旁边多出一层没编号的文件，看着就乱了。
