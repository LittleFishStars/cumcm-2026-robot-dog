#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""T3_figures.py —— 由问题三的运行结果生成论文用数据型图表（PDF 矢量图）
...
# 中文字体：matplotlib 默认配置目录不可写，改用临时目录避免告警
...
def fig_coverage(fig_dir: Path, data_dir: Path) -> None:
    ...
    for i, (wx, wy) in enumerate(way):
        ax.add_patch(plt.Circle((wx, wy), T.COVER_RADIUS, fill=False,
    ...
    ax.set_xlabel("x / m")
    ax.set_ylabel("y / m")
    ax.set_title(f"覆盖路点布局（{len(way)} 个，半径 {T.COVER_RADIUS:.0f} m）", fontsize=10.5)
    ax.legend(fontsize=9, loc="upper right")

    ax = axes[1]
    ax.hist(d, bins=120, color=C_BAND, edgecolor=C_SRC, linewidth=0.4)
    ax.axvline(T.COVER_RADIUS, color=C_SRC, ls="--", lw=1.2)
    ax.text(T.COVER_RADIUS - 8, ax.get_ylim()[1] * 0.95, f"设计半径 {T.COVER_RADIUS:.0f} m",
