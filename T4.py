...
* 检测阶段**复用问题三的 7 个覆盖基点，并按"尽量减少路程"优化**：加 12 个均匀方位外圈点
  （半径 1850 m = 贴边源迎光区内沿，起始角 7.5° 与内圈错开），共 20 个测量位置（原点起点 +
  7 基点 + 12 外圈），访问里程 16 995.65 m。实测听到率 99.9891%（4 224 640 个算例漏 460：
  400 万蒙特卡洛漏 320 + 精细对抗漏 140 + 贴边对抗 0 漏，见
  cumcm.t4.sweep.verify_hearing_stats），**非严格保证**（残余漏例集中在"内部半径源 + 波束
  朝外 + 方向恰落基点方位空隙"，约 0.011%）；20 局演练 256/256 全清、平均虚拟时间 6 445 s。
