...
* **动作记录 schema**（`actions` 里的键）被 `common.trajfigure` 的轨迹图与轨迹表按名读取，
  而轨迹图/轨迹表又是公共代码 —— 谁改了键名却忘改另一处，图上就会静默少一类点；
* **一步扫描的记录 schema**（`scan_steps` 里的键）被 `common.scanfigure` 的逐步扫描图读取，
  同理；
...
class ActionRecorder:
    ...
    def _polar_deg(p: Sequence[float]) -> float:
        ...
        对不上"的假不一致（同类教训见 common.geometry.dist）。
        """
