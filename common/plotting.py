...
1. 字体。默认字体里没有汉字，图上的中文会变成方框。这里给出一份候选字体链，本机装的是
   Noto Sans CJK，Windows 上是微软雅黑、黑体，并统一关掉负号被换成方框的问题。
2. 配置目录。matplotlib 的默认配置目录 `~/.config/matplotlib` 在只读 home 或沙箱里不可写，
   于是它退化成 /tmp 下的随机目录，打印警告，还每次重新扫字体，很慢。这里显式 `setdefault`
   到一个固定可写目录，警告没了，字体缓存也复用上了。

确定性方面，`save_png` 会去掉 PNG 里时间戳一类的元数据，所以同一份输入两次出图逐字节一致。
这是"结果可复现"验收的一部分。
"""
