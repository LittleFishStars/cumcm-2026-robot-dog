...
所有函数都不含随机性，并列时按编号取小，所以同一输入必得同一输出，这是可复现性的要求。
"开路径"指从给定起点出发走遍所有点，终点自由，不要求回到起点，本题两阶段的路线都是这种。

仓库历史上 T3.py 和已删除的 T3_ga.py 各带一份 2-opt，策略不同，数值结果都已经写进报告，
所以这里同时保留并各自命名，不做合并。合并会改变已公布的里程与时间。两个变体分别是：

* `two_opt_first`：发现第一处改进就翻转并 break 内层循环，再从头重扫。
* `two_opt_greedy`：扫描过程中一旦发现更短就把当前解替换掉并继续往后扫，不回退，
  整轮无改进才结束。

问题四的 sweep 与公共精确算子的降级路径都在用它们。两者的共同前提是"距离矩阵 D 的第 0 行
代表起点"，见 `dist_matrix`。
"""
