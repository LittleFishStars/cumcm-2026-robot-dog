...
为什么要逐步骤出图：一整局的轨迹图信息密度太高。上百次测向、几十次清除尝试全挤在一张
3600 m 见方的圆域里，看不出"某一步到底听到了什么、哪些频道还是空白"。而策略的全部信息都
来自这一步步扫描，每步单独出图既能讲清信息是怎么积累起来的，也便于逐站复核，图上每个点
都能与过程日志、api_calls.jsonl 对上。

一步扫描是什么：机器狗停在某个位置，把"尚未采够示向度且未清除"的频道按频道号升序测一遍。
T3 的一步就是起点全频道扫描加每个巡视站各一次。

本模块只负责画，不负责取数。调用方把自己的记录整理成 `ScanStep` 传进来，`scan_step_of` 是
从策略记录构造它的公共入口，两族画法共用同一套视觉约定。

画法分块：`draw_scan_step` 只做编排，每一类元素一个 `_draw_*` 私有函数，覆盖圆域、覆盖圆、
行驶路径、本步测向、就地清除、估计区域、真值源、收尾排版。块的先后顺序就是叠放顺序，
与图例顺序一致。

matplotlib 只在绘图函数内导入，没装时只会抛 `ImportError`，由调用方忽略。官方测试机上没有
matplotlib 也能正常完成整局，`.venv/bin/pip install matplotlib` 即可启用。出图去掉时间戳
一类元数据，同一输入两次出图逐字节一致。
"""
...
from common.plotting import (C_COVER, C_DIR, C_FRAME, C_HIT, C_MEAS, C_NEAR,
