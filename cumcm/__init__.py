...
--------
    ...
    cumcm/t3/         问题 3：确定性策略（本包的主线，与 T3.py 入口对应）
    cumcm/t3ga/       问题 3：GA 对照方案（与 T3_ga.py 入口对应）
    cumcm/analysis/   验证脚本与论文出图

顶层仍保留同名薄入口（T1.py / T3.py / T3_ga.py / T3_validate.py / T3_figures.py / sim_api.py），
所以原有的命令行用法一字不改：
...
分层原则：**依赖只能向下**。common 不 import 任何题目模块；t1/t3/t3ga 可 import common；
analysis 可 import 全部。这样"抽公共工具"不会反过来把某一题的实现细节传染给另一题。
"""

__all__ = ["common", "t1", "t3", "t3ga", "analysis"]
__version__ = "0.2.0"
