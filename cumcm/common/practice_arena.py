...
T3.py 与 T3_ga.py 原先各写了一份几乎相同的 PracticeArena，只有两处差异，这里合并为一份
**取两者之并**的实现（对两边都是严格增强）：

* 复用已在运行的实例（T3.py 的 `reuse_existing`）—— 多会话并行演练时避免抢端口；
* 连续开局的 409 重试（T3_ga.py 的 START_RETRIES）—— 上一局会话未完全释放时短暂等待重试。
