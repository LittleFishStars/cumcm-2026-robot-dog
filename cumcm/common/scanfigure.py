"""单步扫描结果图：把"某一步扫描"看到的信息单独画成一张图（t3 与 t3ga 共用）。
...
一遍。确定性方案 T3 的一步 = 起点全频道扫描 + 每个巡视站各一次；GA 方案同理。

本模块只负责画，不负责取数：调用方把自己的记录整理成 `ScanStep` 传进来。这样两族策略
（数据结构差别很大 —— T3 记逐次动作明细，T3 的 GA 对照方案只记轨迹点与命中点）都能复用
同一套画法，图上的视觉约定也就天然一致。
...
def draw_scan_step(out_path: Path, step: ScanStep, *,
    ...
    - `fonts`：中文字体候选链。两族历史上用的字体不同（GA 版优先 Windows 自带字体），
      传各自的链以保持与既有产物一致；不传则用公共默认链。
