...
`cumcm.t1.TriangulationRegion` 完全一致 —— 区别只是这里要**成千上万个候选点**各算一次，故
...
用采样点的精确最坏距离，见 `cumcm/t2/score`）。

依赖 shapely>=2.1、numpy；命令行自检见文件末尾 `python -m cumcm.t2.region`。
"""
...
from cumcm.t2.config import (BEARING_ERROR_DEG, D_HI, D_LO, DIAM_SENTINEL, PARALLEL_EPS,
...
def feasible_lens(site: Sequence[float], theta1: float, d_lo: float = D_LO, d_hi: float = D_HI,
    ...
    仅用于出图与结果描述：判据本身是"采样源点上的最坏距离 ≤ receive_min"（见 cumcm.t2.score），
