...
# 滚动重规划：只有"已听到、尚未清除且位置 1σ ≤ RHO_GATE"的源才会被当作可插入任务，插进
# 当前巡视路线顺路清除。它是"估计够不够可信、值不值得绕过去清"的门：
#   太大 → 估计不可信，插进来大概率白跑一趟；
#   太小 → 绝大多数源只能等巡视结束再统一清，退化成原来的分段式。
# 与 GEOM_SIGMA=40 分开：后者判"交会几何是否病态、要不要补测垂直视角"，口径更严。
RHO_GATE = 150.0                # 滚动重规划中"值得顺路清除"的位置 1σ 上限 / m

# 滚动重规划的"少花冤枉时间"阈值（详见 cumcm/t3ga/strategy._rolling_survey_clear）：
#
# PRECISE_SIGMA：位置 1σ 已小到可靠清除量级时，停止继续为该频道测向。继续测只能把 σ 从
#     20 m 压到 15 m，却要在每个路点多花 5 s 测向 + 1 s 换频；而收尾的 localize_all /
#     _clear_all 仍是 100% 清除的兜底，故提前停测**不影响"不漏源、必清除"的保证**。
#     与 GEOM_SIGMA=40 同量级（r1800 上实测算例：σ≤40 的频道真实误差中位数仅 4~6 m），
#     实测 20 局虚拟时间 4220→4135 s、测向 2816→2663 次，且 256/256 全清、最坏误差不变。
#     0 表示关闭（旧行为：一直测到 OBS_PER_SOURCE 条为止）。
PRECISE_SIGMA = 40.0
