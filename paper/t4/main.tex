...
%  在 paper/t4/ 目录下运行：xelatex main.tex（跑两遍解决交叉引用/目录）
%  图片在 paper/t4/figures/，由 paper/t4/make_figures.py 读当前求解产物生成
%  （图里的数字全部来自 results/t4/，不写死；矢量 PDF + 两张 NN 对比 PNG）
...
% ---------- 图：统一放 paper/t4/figures/（由 make_figures.py 读当前产物生成） ----------
...
\abstractcn{%
  ...
  $4.3$ 万 $+$ 精细对抗 $18$ 万）实测听到率 $99.9891\%$、贴边对抗 0 漏、访问里程
  $16\,996$ m。定位只用 \texttt{direction}/\texttt{near} 两类硬约束求交
  （\texttt{no\_signal} 不作为约束），配合示向度交会补测；清除以"途中顺路清除（站间
  方位扇区）+ 就近试清 + 兜底逼近"三级机制完成。$20$ 局固定 seed 演练
  \textbf{$256/256$ 全清、0 失败}，平均虚拟耗时 $6445$ s（$\approx 107$ min）、测向
  $247$ 次/局、清除点定位误差均值 $7.66$ m（全部 $\le 20$ m）。时间优化逐旋钮验证并
  收敛到 $6445$ s 实测下界，神经网络独立搜索交叉验证该下界可信；全程确定性可复现、
  离线模拟器对拍证据链完整。%
}{%
