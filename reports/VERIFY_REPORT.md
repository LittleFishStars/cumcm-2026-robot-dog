...
PDF 去时间戳可逐字节复现）；每张图的图例/标题文字均由脚本按产物动态拼装。

## 论文图视觉抽查（视觉模型子代理，三轮 + 程序化复核）

本机模型不能读图，因此视觉抽查交给具备图像输入能力的子代理（`tingly-box/qwen3.8-flash`）：
把 8 张图按 110 dpi 渲染成 PNG 后逐张放大目视，并用像素色值比对确认遮挡（例如折线被半透明图例
压住后颜色由实红 $(192,57,43)$ 变淡为 $(242,215,212)$）。三轮结论如下。

**第一轮：6 处缺陷**（`fig_t4_first_heard`、`fig_t4_nn_layout_compare` 通过）

1. `fig_t4_directional_concept`：120°/60° 方位标注被顶出坐标区、压在标题上；
2. `fig_t4_episode_clear`：右上图例盖住最高柱柱顶与其红色 ✖ 标记；
3. `fig_t4_localization_error`：紫色"最差单源"虚线斜穿图例框；
4. `fig_t4_cost`：左上图例右下角压住总时间折线上升段；
5. `fig_t4_sweep_lattice`：淡红半圆盘与绿色虚线内切圆被坐标区上边界裁断（不闭合）；
6. `fig_t4_nn_speed_compare`："最小 6445 s" 标注越出右边框约 25 px；三根浅灰柱上的白字对比度仅约
   2.6:1；且听率两位小数把 $99.9974\%$ 四舍五入成 $100.00\%$。

**修复**（只改 `paper/t4/make_figures.py` 的版面参数，数据口径与取值未动）：5 张图的图例统一移到
坐标区下方、柱顶留白 $+4$、定位误差图顶部留白 $25\%$、扫描布局 `xlim/ylim` 由 $\pm2500$ 放宽到
$\pm2650$、角度标注环半径 $1580\to1250$（`ylim` 同步到 $\pm1450$）、听率标签改 3 位小数、
"最小"标注移进坐标区、浅色柱改深色字（对比度 $\approx5.4:1$）。

**第二轮：7/8 通过**。仅 `fig_t4_directional_concept` 仍有问题——标题遮挡修好后，右上图例改由环顶
90° 数据点承担遮挡（12 个环点只检出 11 个）。→ 该图图例也移到坐标区下方。

**第三轮：8/8 通过**。12 个环点齐全（90°、60° 标注均为纯黑 $(0,0,0)$，不再被压）、下方图例与横轴
刻度/标签无重叠且未被画布裁掉、无新增遮挡；同时发现一处**既有**小瑕疵：210° 标注被背光侧红色
标记圆盘压住（只余尾部 `0°` 可读）→ 该标注沿径向外推到 $1480$ m（数据点仍在 $1250$ m 环上）。

**程序化复核**（`pdftoppm -r 110` + scipy 连通域，脚本一次性执行后清理临时文件）：
背光侧红色圆盘 bbox $x\,187$--$200$、$y\,325$--$338$ 内深色文字像素 $=0$（标注不再被压），圆盘外侧
左下方文字像素 $=69$（标注可见）；半径 $183\pm12$ px 环上识别到 12 个方位点（$0^\circ$--$330^\circ$
每 $30^\circ$ 一个）全部齐全。
