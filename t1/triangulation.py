...
class TriangulationRegion:
    """交会定位区域：累积检测点，每个点带坐标和示向度，然后求定位区域凸多边形及其几何量"""
    ...
    def add_node(self, x: float, y: float, theta: float) -> TriangulationRegion:
        """加入一个检测点：坐标为 (x, y) 米，theta 是该点测得的示向度，单位为度
    ...
    def from_nodes(cls, nodes: list[tuple[float, float, float]], err: float = 1.0,
        ...
        Args:
            nodes: 检测点列表，每项是 (x, y, theta)，坐标为米、示向度为度
    ...
    def vertices(self) -> list[tuple[float, float]]:
        """定位区域顶点坐标列表 [(x, y), ...]，统一成逆时针。区域为空或退化时返回 []"""
        region = self.region
    ...
    def diameter(self) -> float:
        ...
        区域是凸多边形，所以直径就是顶点之间的最大两两距离，用 numpy 的成对距离矩阵一次求出。
        """
    ...
    def bounded(self) -> bool:
        """定位是否只靠检测点就已经确定，也就是区域没有被目标圆域截断

        有顶点落在圆域边界上，说明那个方向的约束不足，返回 False。只测一次示向度，或者检测点
        与干扰源近似共线，都会落到这种情形。区域为空同样返回 False。
        """
        pts = self.vertices
    ...
    def enclosing_circle(self) -> tuple[float, float, float] | None:
        ...
        由 GEOS 的最小包围圆给出。问题 1 第二问的判据"以定位区域直径为直径的圆能否覆盖此定位
        区域"就是看 r <= diameter / 2。
        """
    ...
    def bearing(a: tuple[float, float], b: tuple[float, float]) -> float:
        """在 a 点观测 b 点的方位角，单位度，取值范围 [0, 360)：x 轴正向逆时针转到 a→b 的夹角"""
        return degrees(atan2(b[1] - a[1], b[0] - a[0])) % 360.0

    def __repr__(self) -> str:
        """对象的可读表示：检测点数、顶点数、直径，以及是否仅靠检测点就能确定"""
...
def _demo() -> None:
    ...
    手上没有实测数据，先假定干扰源真值 G，再按几何关系反推各点的示向度，这样区域保证非空。
    """
    g = (500.0, 400.0)
