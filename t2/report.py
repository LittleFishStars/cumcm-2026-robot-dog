"""问题二的成果落盘：CSV 适合度表、JSON 结论，还有控制台报表。

三份产物各有各的用途，谁也替不了谁。

`t2_suitability.csv` 是全域粗网格的逐行记录，缺省 25 m 一格，每行给 x、y、到源集的最坏距离、
可测性、最坏定位直径 J、适合度 F。图上的每个色块都能在这张表里找到对应行，"图数据一致"就
靠它。

`t2_second_site.json` 装结论和全部校验：最优第二检测点、极坐标、候选区域两瓣的范围与面积、
可行域面积、最坏情形细节、shapely 对照、离散化加密对照、弧带连续性。报告里引用的每个数字都
从这里取，免得正文数字和程序输出对不上。

控制台报表是跑一次就能看懂的 6 行小结加校验状态。出现异常值时直接给 ⚠，不悄悄放过。

CSV 的主键写的是最坏定位直径而不是适合度，因为 J 才是算法的原始量，F 只是它的归一化。
"""
