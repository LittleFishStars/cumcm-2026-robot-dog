"""问题二的求解算法。从源不确定集到可测性保证，再到最坏定位直径与候选区域，一路推下来。

三个环节依次对应题面那句"给出第二个检测点的选择策略……并给出第二个检测点的候选区域"。

先是源不确定集，见 `source_samples`。只测了一次示向度，源的位置只知落在楔形
C = { d ∈ [D_LO, D_HI]、相对示向度偏离 |δ| ≤ 1° } ∩ 目标圆域内。D_HI = 1500 m 来自"能被 S1
听到"，D_LO = 5 m 来自附录 2(9)，近距测不到示向度。C 是连续集，算法用 d×δ 的确定性网格把它
离散掉，网格里带上 4 个角点 `corner_sources`，最坏情形就出在角上。

再是可测性保证，落在 `coarse_fields` 的 feasible 上。第二点要测到源须 |S2G| ≤ R_rec，
R_rec ∈ [1000, 1500] 逐源固定但未知，所以取最保守的 1000 m，还要求对 C 中每个采样点都成立。
这样得到的可行域是三个圆盘之交，圆心分别是 S1 与楔形远端的两个角，半径都是 1000 m。它是圆域内
很小的一块透镜，面积 0.45 km²，约占圆域的 4e-5。第二点落在透镜之外，一旦源比预想更近就什么都
测不到，第二次测向白跑一趟。

最后是最坏定位直径与最优区域，见 `worst_case_diameters` 与 `solve`。选定 S2 后两次测向把源约束
在一个四边形内，就是附录图 2 那个，用它的直径度量定位效果，与问题 1 同口径。源在 C 内的位置和
第二次测向误差 δ₂ ∈ [−1°, 1°] 都未知，那就取最坏情况：

       J(S2) = max_{G ∈ C, |δ₂| ≤ 1°} 直径( 楔形(S1, θ1) ∩ 楔形(S2, θ₂+δ₂) ∩ 圆域 )

问题二到这里就化成"在可行域上最小化 J"。适合度 F(S2) = J*/J(S2)，落在 (0,1] 内，1 表示最优。
候选区域取 { F ≥ 1/(1+η) }，缺省 η = 10%，也就是 J 不超过最优值的 1.1 倍。

算法结构如下，全程确定性，没有随机数。第一步全域粗搜，直角网格缺省 25 m，算可测性与 J，取
可行域内最小点。第二步局部细化，以第一步的解为中心，±80 m、步长 2 m，直角网格再搜一遍，J 在
最优点附近光滑，25 m 粗网格必然把最优圈进窗口。第三步求最优区域，在绕 S1 的极坐标网格上取
"可行 ∩ J ≤ (1+η)J*"的点，逐方位角求径向区间，再组装成弧带多边形；解关于示向度方向镜像对称，
所以通常是两瓣。第四步校验，解析构造对 shapely 精确直径、最坏情形用 shapely 复算、离散化加密
复算、圆域截断与弧带连续性，全部写进 `checks`；缺省还做一次抽样校验，见 `solve` 的 verify_n。

命令行入口在 `t2/cli.py`，也就是 `python -m t2`。
"""
