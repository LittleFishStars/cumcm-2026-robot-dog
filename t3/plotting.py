...
绘图放在 `/exit` 之后，`save_trajectory` 由运行编排在收尾时调用，不占用现实时间预算，
也不影响任何实时决策。图上画的就是 `RobotDog.actions` 里记录的实际动作点，可与过程日志
逐点对账。同名 CSV 轨迹表让"图上每个点"都能被逐行复核。

画的东西：作业圆域 1800 m 与源生成域 1770 m；7 个半径 1000 m 的覆盖圆和它们的圆心，圆心
标着访问序号；从原点起的行驶路径；按结果分类的动作点，分测得示向度、无信号、近距、清除
尝试、清除成功五类；干扰源真值和 20 m 清除半径。

与问题四共用的那部分画法都在 `common.trajfigure`，动作点分类、真值源、清除半径小圆、图例
排版、轨迹表与落盘编排。本模块只管问题三独有的元素，也就是 7 个覆盖圆与圆心访问序号，
外加问题三口径的图例文字。
...
from common.plotting import (C_COVER, C_FRAME, C_PATH, C_SRC, font_context, no_plot_hint,
                                   save_png, setup_mpl_env)
from common.scanfigure import (STEP_DIR_NAME, draw_scan_step, scan_figure_path,
                                     scan_step_of)
from common.trajfigure import (MEASURE_LABELS_T3, plot_action_points,
                                     plot_source_markers, truth_points)
from common.trajfigure import save_trajectory as _save_trajectory
from t3.config import (CLEAR_RADIUS, COVER_RADIUS, RECEIVE_MAX, REGION_RADIUS,
                             TRAJ_DIR, TRAJ_DPI)
from t3.covering import CoverPlan
