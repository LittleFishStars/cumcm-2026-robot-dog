...
运行（示例）：
    python -X utf8 -m t4.nn_layout --gen 4000 --mc-n 20000 --train-epochs 80 \
...
def _edge_miss_fast(layout: np.ndarray, n_azi: int = 90,
    ...
    算例集合由 `t4.sweep.adversarial_cases` 按参数缓存（与布局无关），判据走批量路径，
...
def ga_search(net: nn.Module, n_free: int, n_pop: int = 300,
    ...
    """
    from t4.sweep import measure_layout
    rng = np.random.default_rng(seed)
