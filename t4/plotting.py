...
与问题三共用的那部分画法在 `common.trajfigure`：动作点分类、真值源与波束方向短射线、清除
半径小圆、图例排版、轨迹表与落盘编排。本模块只管问题四独有的东西：测量位置、
定向源 ±90° 波束扇形，还有图例里"测向"口径的文字。

逐步扫描图放在 <save-dir>/scan/ 下，复用 common.scanfigure 的通用画法。覆盖圆信息对问题四
没有意义，这里本来就没有 7 个覆盖圆，所以传空，只保留该步测向点、示向度射线、行驶路径、
估计区域与真值。
"""
...
from common.plotting import (C_FRAME, C_GRAY, C_PATH, C_SRC, font_context, no_plot_hint,
                                   save_png, setup_mpl_env)
from common.scanfigure import (STEP_DIR_NAME, draw_scan_step, scan_figure_path,
                                     scan_step_of)
from common.trajfigure import (MEASURE_LABELS_T4, plot_action_points,
                                     plot_source_markers, truth_points)
from common.trajfigure import save_trajectory as _save_trajectory
from t4.config import (CLEAR_RADIUS, DIR_BEAM_HALF_DEG, RECEIVE_MAX, REGION_RADIUS,
                             TRAJ_DIR, TRAJ_DPI)
