"""扫描布局：原点 + 7 覆盖基点 + 12 个均匀方位外圈点，问题四检测阶段用。

定向源在检测点 p 处可被听到，等价于

    |p − g| ≤ R  且  (p − g) · u(θ) ≥ 0        R 为接收半径 1000~1500，θ 为定向方向，含边界

也就是说检测区是"接收圆 ∩ 前向半平面"，一个半径 R 的半圆盘，圆心 g、半径 R、朝向 θ 全部
未知。问题三的 7 点覆盖只保证"圆域内任意点到最近巡视点 ≤ 1000 m"，那对全向源必听到；换到
定向源就不成立，背对着所有巡视点的源哪怕近在咫尺也听不到，引擎返回 no_signal，见 engine.py
的 _in_directional_coverage。这就是"搜索不到还有可能是方向不对"的由来。

本模块的布局，最终定案 20 个测量位置：

  1. 原点做起点全频道扫描，位置成本为零；
  2. 7 个覆盖基点就是问题三的巡视站 t3.config.SURVEY_CENTERS，直接复用，负责全向覆盖保证；
  3. 12 个均匀方位外圈点，半径 OUTER_RING_RAD = 1850 m，MID_RING_N = 0，不加内部补点。起始
     方位自 0° 旋转 OUTER_RING_ROT_DEG = 7.5°：2026-09-12 定案，与内圈基点方位错开、方位互补。
     两组 seed A/B 复核 0°→7.5°：seed 2026-2030 时间持平、误差 8.61→7.15 m，seed 2031-2035
     时间 6858→6411 s（−447 s）、里程 −1880 m。

为什么外圈取 1850 m、12 个方位？这是对"7+21 中点外推"布局的结构优化：

  * 半径 1850 m：任意源的迎光区在源前方 [|g|, |g|+R] 的纵深里；源最远半径 1770，所以任意贴边
    源的迎光区内沿 = 1770 m，外圈设在其内侧 1850 m 时径向偏差 < 80 m。原"中点外推"布局把多数
    点推到 2270 m，反而离内部源的迎光面太远，实测只剩 99.81%；1850 m 单圈加 12 个方位同时
    覆盖贴边源与内部半径源的迎光区，实测 99.9891%，4 224 640 个算例漏 460，其中贴边对抗
    4.3 万例 0 漏；
  * 12 个方位，间隔 30°：相邻外圈点最大距离 ≈ 1850·sin15° ≈ 479 m，不到接收半径的一半，任何
    方向的源都能被方向差 ≤ 15° 的外圈点听到。为什么不是更少的点？试过 11 点，间隔 32.7°、
    布局路线 16 725 m，但方位间隔变宽使顺路清除的方位扇区也变宽、绕路更多，实战总里程反升
    464 m。12 点是"减少路程"的实战最优；
  * 里程：20 个测量位置的最短开放路径 16 995.65 m，外圈起始角 7.5° 与内圈错开，比"7+21 中点
    外推"的 29 点 21 580 m 省 21%，还少 9 个测量位置；
  * 为什么不加内部补点，20 点定案：曾试过加 3 个内部补点，r = 850 m，共 23 个位置，听率提到
    99.9974%，但 20 局演练实测多 ~34 次测向、慢 80 s。20 点版听率 99.9891%，422.5 万算例漏
    460、贴边对抗 0 漏，对每局 16 个源的实战任务 20 局实测 256/256 全清，耗时与完成率双优。

到这里仍是统计口径上的保证，不是严格的。残余漏例约 0.01%，论文按实测统计报告，也就是
verify_hearing_stats 那套数，不声称严格不漏。
"""
